...
sidebarLayout(
  ...
  mainPanel(
    ...
    wellPanel(
      textOutput("txt_reporte_neuro_model")
    )
  )
)
  ),
...
observeEvent(input$run_neuro_model, {
  output$txt_reporte_neuro_model <- renderText({
    ...
    tryCatch({
      ...
      reporte <- paste0("### RESUMEN DEL MODELO ESTRUCTURAL ###\n\n",
                        "Objetivo: Evaluar el efecto sobre '", 
                        target_label, 
                        "'.\n",
                        "Método: Se aplicó una ", 
                        model_type, 
                        " ajustada por ", 
                        length(pred_vars), 
                        " variables.\n\n")
      ...
      if(length(sig_rows) > 0) {
        reporte <- paste0(reporte, "RESULTADOS CLAVE:\n")
        for(row in sig_rows) {
          ...
          if(is_binary) {
            ...
            reporte <- paste0(reporte, 
                              "- La variable '", 
                              clean_label, 
                              "' es significativa (p = ", p_val, 
                              "). Por cada unidad de incremento, la probabilidad del evento aumenta ", 
                              or_val, " veces (Odds Ratio).\n")
          } else {
            coeff <- round(stats[row, 1], 2)
            reporte <- paste0(reporte, "- '", 
                              clean_label, 
                              "' muestra una asociación significativa (p = ", p_val, 
                              "). Un aumento en esta variable se traduce en un cambio de ", coeff, 
                              " unidades en '", target_label, "'.\n")
          }
        }
      } else {
        reporte <- paste0(reporte, "RESULTADOS: No se detectaron asociaciones con significancia estadística (p < 0.05) para los predictores seleccionados en esta muestra.\n")
      }
      
      if(!is_binary) {
        reporte <- paste0(reporte, "\nAJUSTE: El modelo explica el ", r_sq, "% de la variabilidad observada.")
      }
      
      return(reporte)
      
    }, error = function(e) {
      return(paste("Error en el procesamiento: Los datos seleccionados no permiten la convergencia del modelo.",
                   "Verifique que las variables tengan suficientes casos válidos (N).", e$message))
    })
  })
})
