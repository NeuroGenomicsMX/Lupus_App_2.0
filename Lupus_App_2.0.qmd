...
La Pestaña 1 ofrece un reporte general descriptivo del Registro Mexicano
de Lupus orientado a personas no técnicas. La salida es 100% texto: no
hay gráficas en esta pestaña.

La persona usuaria:

1. Selecciona una o varias variables del registro que quiere ver
   descritas. El selector incluye todas las variables relevantes con
   etiquetas legibles en español; los nombres internos no cambian.
2. Presiona "Generar reporte" para producir una narrativa.

La aplicación devuelve un reporte narrativo en español sencillo. Cada
variable se describe de la forma más interpretable posible para
personas no técnicas, evitando entregar promedios o rangos numéricos
crudos que no aportan significado clínico:

- **Variables de conteo / tiempo** (edad, años con LES, número de
  tratamientos): se reportan promedio, mediana y rango porque el
  promedio sí tiene sentido para estas variables.

- **Índices con umbral clínico validado** (SLEDAI, SLICC, PSQI,
  calidad de vida WHOQOL, salud mental y dolor SF-36): se reporta el
  porcentaje de personas en cada categoría clínica definida en el
  diccionario de variables, no la media. Por ejemplo, SLEDAI ≥ 6 =
  enfermedad activa; PSQI <5 sin problemas, 5-7 atención médica, 8-14
  evaluación clínica, ≥15 alteraciones graves.

- **Puntajes SCL-90R** (depresión, ansiedad, somatización, etc.): cada
  uno tiene un rango validado en el data curating (p. ej. depresión
  12-60, ansiedad 9-45). Se reportan los terciles del rango teórico de
  la escala (bajo / intermedio / alto), recordando a la persona que
  puntajes más altos indican mayor severidad sintomática. No se
  reportan medias ni rangos numéricos sin contexto.

- **Variables de daño orgánico** (Daño ocular, renal, pulmonar,
  cardiovascular, cardiomiopatía, vascular periférico, gastrointestinal,
  musculoesquelético, cutáneo, fallo gonadal prematuro, diabetes,
  cáncer): según el data curating son **binarias** (1 = presente, 0 =
  ausente). Se reportan como porcentaje de personas con daño en ese
  órgano, NO como media numérica.

- **Variables categóricas** (sexo, escolaridad, estado, comorbilidades,
  tipo de corticoesteroide, etc.): la(s) categoría(s) más frecuente(s)
  con su porcentaje, y total de personas con respuesta.

- **Variables lógicas (Sí/No)**: proporción de "Sí" del total.

Los valores calculados (porcentajes, conteos, promedios cuando aplican)
se resaltan visualmente con un fondo morado suave para que la persona
note de un vistazo qué cambia cuando ajusta la selección de variables.
...
## Pestaña 1: Procesamiento de datos especifico
  ...
  nav_panel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        selectInput(
          inputId = "comp_var1",
          label = "Variable Categórica de Agrupación (Eje X / Grupos):",
          choices = c(
            "Sexo" = "Sexo",
            "Actividad del LES" = "Actividad_del_LES",
            "Padece Diabetes" = "Padece_diabetes",
        ...
        selectizeInput(
          ...
          choices = diccionario_reporte_gen, # Reutiliza tu catálogo general de variables
          selected = "Edad"
        ),
        hr(),
        # Botón de descarga unificado con estilo dorado institucional
        downloadButton("download_comp", "Descargar Análisis (.txt)", class = "btn-success")
      ),
...
server <- function(input, output, session) {
  ...
  analisis_comparativa <- reactive({
    ...
    # Criterio automatizado: ¿La segunda variable se comporta como categórica?
    es_categorica_v2 <- is.factor(v2) || is.character(v2) || length(unique(v2)) <= 5
    
    if (es_categorica_v2) {
      # ================================================================
      # ESCENARIO A: Categórica vs Categórica -> Prueba de Chi-Cuadrada
      # ================================================================
      tabla_contingencia <- table(v1, v2)
      prueba_chi <- tryCatch(chisq.test(tabla_contingencia), error = function(e) NULL)
      ...
      if (is.null(prueba_chi)) {
        ...
        texto_html <- paste0(
          ...
          "<p><b>Interpretación clínica simple:</b> Tomando un nivel de confianza estándar del 95%, se concluye que <b>", significativo, "</b> una asociación estadísticamente significativa entre las variables seleccionadas. ",
          ifelse(p_val < 0.05, 
      ...
      # Gráfico de barras con paleta Viridis
      p_grafico <- ggplot(
        df_clean,
        aes(
          x = factor(!!sym(input$comp_var1)),
          fill = factor(!!sym(input$comp_var2))
        )
      ) +
        geom_bar(
        ...
        labs(
          y = "Frecuencia",
          x = paste("Grupos de:", input$comp_var1),
          fill = input$comp_var2
        ) +
      ...
      # ================================================================
      # ESCENARIO B: Categórica vs Continua -> Prueba de Permutaciones
      # ================================================================
      df_clean[[input$comp_var1]] <- factor(df_clean[[input$comp_var1]])
      
      # Ajustar el modelo lineal observado para extraer el estadístico F clásico (Análogo a ANOVA)
      fit_obs <- tryCatch(lm(as.formula(paste(input$comp_var2, "~", input$comp_var1)), data = df_clean), error = function(e) NULL)
      
      if (is.null(fit_obs) || is.na(summary(fit_obs)$fstatistic[1])) {
        return(list(
          tipo = "error",
          texto = "<p>Imposible procesar el modelo matemático de varianza. Verifique la dispersión de los datos continuos.</p>",
          plot = NULL
        ))
      }
      
      f_observado <- summary(fit_obs)$fstatistic[1]
      
      # Algoritmo de Permutaciones Monte Carlo (500 réplicas para balancear precisión y rapidez web)
      set.seed(42)
      f_permutados <- replicate(500, {
        y_barajeada <- sample(df_clean[[input$comp_var2]])
        fit_perm <- lm(y_barajeada ~ df_clean[[input$comp_var1]])
        f_val <- summary(fit_perm)$fstatistic[1]
        if (is.na(f_val)) return(0) else return(f_val)
      })
      
      p_val_empirico <- mean(f_permutados >= f_observado, na.rm = TRUE)
      significativo <- if (p_val_empirico < 0.05) "SÍ existen" else "NO existen"
      
      texto_html <- paste0(
        "<p><b>Prueba Estadística Ejecutada:</b> Prueba de Permutaciones exacta mediante simulación Monte Carlo (500 iteraciones).</p>",
        "<p><b>Muestra Analizada:</b> n = ", nrow(df_clean), " registros analizados.</p>",
        "<p><b>Resultado del Modelo:</b> Estadístico F de referencia = ", round(f_observado, 3), 
        ", p-valor empírico = <b>", format.pval(p_val_empirico, digits = 4), "</b>.</p>",
        "<p><b>Interpretación clínica simple:</b> Tras simular múltiples escenarios aleatorios, la prueba indica que <b>", significativo, "</b> diferencias estadísticamente significativas entre los promedios de los grupos evaluados. ",
        ifelse(p_val_empirico < 0.05, 
               "El gráfico inferior ilustra cómo las curvas de densidad se distancian entre sí, confirmando un perfil clínico numéricamente diferente según cada estado.", 
               "Las curvas muestran un alto solapamiento visual, lo que confirma que las ligeras diferencias en los promedios numéricos actuales son producto del azar muestral."), "</p>"
      )
      
      # Gráfico de densidad suavizada con paleta Viridis
      p_grafico <- ggplot(df_clean, aes(x = !!sym(input$comp_var2), fill = factor(!!sym(input$comp_var1)))) +
        geom_density(alpha = 0.5, color = NA) + # Alpha ajustado para mejor visibilidad
        labs(y = "Densidad de Distribución", x = input$comp_var2, fill = paste("Grupos de:", input$comp_var1)) +
        scale_fill_viridis_d(option = "viridis", begin = 0.1, end = 0.9) + # <-- NUEVO: Paleta accesible
        theme_minimal(base_family = "sans") +
...
knitr::purl("Lupus_App_2.0.qmd", output = "Server_LupusApp_2.0.R")
```
