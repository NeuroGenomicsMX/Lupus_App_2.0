...
server <- function(input, output, session) {
  ...
  analisis_comparativa <- reactive({
    ...
    if (es_categorica_v2) {
      ...
      # Gráfico de barras apiladas al 100% para evaluar proporciones relativas
      p_grafico <- ggplot(df_clean, aes(x = factor(!!sym(input$comp_var1)), fill = factor(!!sym(input$comp_var2)))) +
        ...
        labs(y = "Porcentaje Relativo dentro del Grupo", x = paste("Grupos de:", input$comp_var1), fill = input$comp_var2) +
        scale_fill_manual(values = c("#5A2A7A", "#E5A823", "#D1C4E9", "#00897B", "#C2185B")) +
      ...
      # Gráfico de densidad suavizada para evaluar solapamiento
      p_grafico <- ggplot(df_clean, aes(x = !!sym(input$comp_var2), fill = factor(!!sym(input$comp_var1)))) +
        ...
        scale_fill_manual(values = c("#5A2A7A", "#E5A823", "#D1C4E9", "#00897B")) +
        theme_minimal(base_family = "sans") +
        theme(legend.position = "bottom", axis.text = element_text(size = 11), axis.title = element_text(size = 12))
