...
server <- function(input, output, session) {
  ...
  analisis_comparativa <- reactive({
    ...
    if (es_categorica_v2) {
      ...
      # ================================================================
      # ESCENARIO B: Categórica vs Continua -> Prueba de Permutaciones Robustas
      # ================================================================
      ...
      if (p_val_global >= 0.05) {
        ...
        texto_html <- paste0(
          texto_html,
          "<p><b>Interpretación clínica simple:</b> Tras comparar a los grupos de la muestra, se concluye que <b>NO existen diferencias estadísticamente significativas</b> (p = ", format.pval(p_val_global, digits = 4), "). ",
          "Las ligeras variaciones visuales en las curvas del gráfico inferior son producto del azar, confirmando que el comportamiento de esta variable es similar entre los grupos analizados.</p>"
        )
      } else {
        ...
        if (length(grupos) > 2) {
          # 2.1 Post Hoc por pares (Si hay 3 o más grupos, ej. Escolaridad, Tratamientos)
          ...
          # Corrección de múltiples comparaciones (FDR - False Discovery Rate)
          ...
          # 2.2 Solo hay 2 grupos (Ej. Sexo, Diabetes). La global ES la post hoc.
          texto_html <- paste0(
            texto_html,
            "<p><b>Interpretación clínica simple:</b> Al contar con solo dos grupos, la prueba confirma directamente que <b>SÍ existen diferencias reales</b> entre ellos. El gráfico inferior ilustra cómo sus distribuciones se separan o concentran en rangos distintos, respaldando esta evidencia matemática.</p>"
          )
        }
      }
      
      # Gráfico de densidad suavizada con paleta Viridis ... 
      # (Tu código de p_grafico <- ggplot(...) original sigue intacto aquí debajo)
